...
import {
  Upload, Image as ImageIcon, File, Trash2, Download, Eye, Copy,
  Video, Film, FolderOpen, Grid, List, Search, SortAsc,
  Play, ImagePlus
} from "lucide-react";
      ...
      const filesWithUrls = await Promise.all(
        (data || []).map(async (file) => {
          ...
          const { data: { publicUrl: thumbUrl } } = supabase.storage
            .from('cms-media')
            .getPublicUrl(currentFolder ? `${currentFolder}/thumb_${file.name}` : `thumb_${file.name}`);
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex border rounded-md">
            ...
            <Button
              ...
              size="sm"
              onClick={() => { setViewMode('slideshow'); setSlideshowIndex(0); }}
            >
          ...
          {filteredFiles.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-muted-foreground mb-4">
                {searchQuery ? 'Try a different search term' : 'Upload your first files to get started'}
              </p>
                      ...
                      <CardContent className="p-0">
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <Button size="sm" variant="secondary" onClick={(e) => { e.stopPropagation(); setSelectedFile(file); setPreviewOpen(true); }}>
                              <Eye className="w-4 h-4" />
                            </Button>
                            <Button size="sm" variant="secondary" onClick={(e) => { e.stopPropagation(); copyUrl(file.url); }}>
                              <Copy className="w-4 h-4" />
                            </Button>
                            <Button size="sm" variant="destructive" onClick={(e) => { e.stopPropagation(); handleDeleteFile(file); }}>
              ...
              {filteredFiles.map((file) => (
                ...
                >
                  ...
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); copyUrl(file.url); }}>
                      <Copy className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); downloadFile(file); }}>
                      <Download className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="ghost" className="text-destructive" onClick={(e) => { e.stopPropagation(); handleDeleteFile(file); }}>
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </div>
                </div>
        ...
        <Card className="overflow-hidden">
          <div className="relative aspect-video bg-black">
            ...
            <Button
              ...
              onClick={() => setSlideshowIndex((slideshowIndex - 1 + filteredFiles.length) % filteredFiles.length)}
            >
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg overflow-hidden bg-muted flex items-center justify-center max-h-96">
                {isImage(selectedFile.type) ? (
                  <img src={selectedFile.url} alt={selectedFile.name} className="max-w-full max-h-96 object-contain" />
              ...
              <div className="flex gap-2">
                <Button onClick={() => copyUrl(selectedFile.url)} variant="outline" className="flex-1">
                  <Copy className="w-4 h-4 mr-2" />
                  Copy URL
                </Button>
                <Button onClick={() => downloadFile(selectedFile)} variant="outline" className="flex-1">
                  <Download className="w-4 h-4 mr-2" />
                  Download
                </Button>
                <Button onClick={() => handleDeleteFile(selectedFile)} variant="destructive" className="flex-1">
